...
public class BinarySearchTree<T extends Comparable<T>> {
	...
	public BinarySearchTree(T[] ar) {
		...
		if (ar.length == 2) {
			this.root.left = new BinaryNode(ar[0]);
			this.root.right = NULL_NODE;
			return;
		} // else
		boolean isOdd = middle % 2 == 1;
		double Dmiddle = ar.length/2;
		double rightAdd = Math.ceil(Dmiddle/2);
		int rightPos = (int) (Dmiddle + rightAdd);
		this.root.left = new BinaryNode(ar[middle / 2]);
		this.root.left.buildTree(ar, middle / 2, middle);
//		if (isOdd) {
//			this.root.right = new BinaryNode(ar[middle / 2 + middle + 1]);
//			this.root.right.buildTree(ar, middle / 2 + middle + 1, middle);
//		} else {
			this.root.right = new BinaryNode(ar[rightPos]);
			this.root.right.buildTree(ar, rightPos, middle);
		//}
	...
	public class BinaryNode {
		...
		public void buildTree(T[] ar, int middle, int lastMiddle) {
			// special cases
			// int arMiddle = (ar.length-1)/2;
			double middleDifference = Math.abs(lastMiddle - middle);
			int range = Math.max((ar.length - 1) - lastMiddle, lastMiddle);

			// base cases
//			if (middle == 0 || middle == ar.length) {
//				this.left = NULL_NODE;
//				this.right = NULL_NODE;
//				return;
//			}
			int difference = (int) Math.ceil(middleDifference / 2);
			int newLeftPos = (middle - difference);
			int newRightPos = (middle + difference);
			if (middle < lastMiddle) {// building on left
				// insert Left
				if (newLeftPos >= 0 /** && newLeftPos < lastMiddle **/
				) {
					this.left = new BinaryNode(ar[newLeftPos]);
					if (difference != 1) {
						this.left.buildTree(ar, newLeftPos, middle);
					}
				...
				// insertRight
				if (newRightPos < ar.length - 1 && newRightPos < lastMiddle) {
					this.right = new BinaryNode(ar[newRightPos]);
					if (difference != 1) {
						this.right.buildTree(ar, newRightPos, middle);
					}
			...
			if (middle > lastMiddle) {// building on right
				// instert left
				if (newLeftPos >= 0 && newLeftPos > lastMiddle) {
					this.left = new BinaryNode(ar[middle - difference]);
					if (difference != 1) {
						this.left.buildTree(ar, newLeftPos, middle);
					}
