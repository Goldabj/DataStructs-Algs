...
public class RandomGraph {
	...
	public TreeSet<Integer> reachableInTwo(int startLabel) {
		...
		if(startVertex.neighbors.isEmpty()) {
			return new TreeSet<>();
		}
		TreeSet<Integer> inTwo = new TreeSet<>();
		// add neigbors neighbors.labels
		for(Vertex neighbor: startVertex.neighbors) {
			for(Vertex nNeighbor: neighbor.neighbors) {
				inTwo.add(nNeighbor.label);
			}
		}
		return inTwo;
	}
