 ...
 *
 * @author Matt Boutell and <<Brendan Goldacker >>>. Created Oct 6, 2014.
 */
public class StringHashSet {
	...
	public boolean add(String item) {
		...
		if (this.stringSet[index] == null) {
			this.stringSet[index] = new Node(item, NULL_NODE); // first node in
																// this position
			this.size++;
			this.numberOfNodes++;
		} else {
			...
			// {
			// currentNode.child.parent = currentNode; // sets the child's
			// parent to this Node
			// }
		}
	...
	private void rehash() {
		...
		for (int i = 0; i < oldSet.length; i++) {
			if (oldSet[i] != null) { // Node in this position
				...
				while (currentNode != NULL_NODE) {// add rehash the chain and
													// add
	...
	public boolean remove(String item) {
		...
		for (int i = 0; i < this.stringSet.length; i++) {
			...
			if (currentNode != null) {
				while (currentNode != NULL_NODE) {
					if (currentNode.data == item) {
						if (currentNode.child != NULL_NODE) {// has Nodes below
																// it
	...
	private class HashIterator implements Iterator<String> {
		...
		public boolean hasNext() {
			if (currentNode.child != NULL_NODE) {
				if (currentNode.child != NULL_NODE) {
					return true;
				}
				int hashCode = stringHashCode(currentNode.data);
				if (hashCode < 0) {
					hashCode += Integer.MAX_VALUE;
				}
				int index = hashCode % capaicty;

				if (index == stringSet.length) {
					return false;
				}

				for (int i = index + 1; i < stringSet.length; i++) {
					if (stringSet[i] != null) {
						return true;
					}
				}
