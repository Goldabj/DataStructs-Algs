...
public class BinaryHeap<T extends Comparable<? super T>> {
	...
	public BinaryHeap(Class<T> classtype) {
		this();
		this.type = classtype;
	}
	/**
...
//		insert(element); // for n log n time
	...
	public T deleteMin() {
		...
		T min = this.heap.remove(1);
		this.size--;
		T newRoot = this.heap.remove(this.size - 1);
	...
	private void perculateDown(int hole) {
		...
		int newHole = hole;
		int minChildPos = 1;
		while(newHole <= this.size-1) {
			T parent = this.heap.get(newHole);
			T minChild = null;
			// find min of children
			int leftChildPos = newHole *2;
			if(leftChildPos <=this.size -1) { // check if this has child
				T leftChild = this.heap.get(leftChildPos);
				
				T rightChild = null;
				if(leftChildPos + 1 <= this.size -1) { // check for right child
