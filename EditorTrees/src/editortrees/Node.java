...
public class Node {
	...
	public Node clone(Node node, Node parent1) {
		if (node == EditTree.NULL_NODE) {
			return EditTree.NULL_NODE;
		}
		// new copy of node
		Node copy = new Node(node.getElement(), parent1);
	 ...
	 * 
	 */
	public int changeBalance(boolean addedRight) {
...
//	}
	...
	// its slow 0(n)
	public void calculateAllBalance() {
		...
		int balance1 = leftSize - rightSize;
		switch (balance1) {
		case 1:
			this.balance = Code.LEFT;
			break;
...
//	}
	...
	public Stack<Node> getSplitStack(Stack<Node> stack, int pos){
		// if equal
		if (this.rank == pos) {
			stack.push(this);
			return stack;
		...
		if (this.rank < pos) {
			stack.push(this);
			return this.right.getSplitStack(stack ,pos - (this.rank + 1));
		}
		// go left
		stack.push(this);
		return this.left.getSplitStack(stack, pos);
	}
	...
	private String getString(int length, boolean beenLeft, boolean beenRight, boolean parentCalledThis,
		...
		if (sb.length() < length) {
			...
			if (sb.length() < length) {
				...
				if (sb.length() < length) {
					if (this.parent.rank!=-1 && !parentCalledThis) {
	 ...
	 * @return
	 */
	public Stack<Node> concatenate(int insideHeight, int smallHeight, Stack<Node> path) {
		path.push(this);
		...
		if (this.balance.equals(Code.LEFT)) {// go right, and subtract 2 from
												// height
			return this.right.concatenate(insideHeight - 2, smallHeight, path);
		}
		return this.right.concatenate(insideHeight - 1, smallHeight, path);
	}

	public Stack<Node> reverseConcatenate(int smallHeight, int insideHeight, Stack<Node> path) {
		path.push(this);
		if(insideHeight-smallHeight <=1){
			return path;
		}
		// else find the height balanced point
		if (this.balance.equals(Code.RIGHT)) {// go left, and subtract 2 from
												// height
			return this.left.reverseConcatenate(insideHeight - 2, smallHeight, path);
		}
		return this.left.reverseConcatenate(insideHeight - 1, smallHeight, path);
	}

	
	/**
	 * recurrsive method that builds up subtrees from strings
	 *
	 * @param left1
	 * @param right1
	 */
	public void constructFromString(String left1, String right1) {
		int leftLength = left1.length();
		int rightLength = right1.length();
		this.rank = leftLength;
		...
		// left build
		if(leftLength > 0) {
			int leftmid = leftLength/2;
			this.left = new Node(left1.charAt(leftmid), this);
			if(leftLength == 1) {
				this.left.balance = Code.SAME;
				this.left.rank = 0;
			}else if(leftLength == 2) {
				this.left.left = new Node(left1.charAt(0), this.left);
				this.left.balance = Code.LEFT;
				this.left.rank = 1;
			}else {// length > 2
				this.left.constructFromString(left1.substring(0, leftmid), left1.substring(leftmid+1));
			}
		}
		// right build
		if(right1.length() > 0) {
			int rightmid = rightLength/2;
			this.right = new Node(right1.charAt(rightmid), this);
			if(rightLength == 1) {
				this.right.balance = Code.SAME;
				this.right.rank = 0;
			}else if(rightLength == 2) {
				this.right.left = new Node(right1.charAt(0), this.right);
				this.right.balance = Code.LEFT;
				this.right.rank = 1;
			}else {// length > 2
				this.right.constructFromString(right1.substring(0, rightmid), right1.substring(rightmid+1));
			}
		}
		// could possible change balances when comparing ranks here make run time aprox = n; 0(n)
		// although if change all balance is called in edit tree then run time is aprox= 2n; 0(n)
	}
