...
public class EditTree {
	...
	public Node root_parent = new Node(); // dummy node that is parent to //
											// root
	...
	public EditTree(String s) {
		...
		// TODO: ask about this
		this.root_parent.left = this.root;// why must we reassign root parents
											// left when roots element is
											// changed??
		if (length <= 1) {// already created to one node, which is the root
			return;
		}
		// else build rest of the tree
		this.root.constructFromString(s.substring(0, middle), s.substring(middle + 1));
