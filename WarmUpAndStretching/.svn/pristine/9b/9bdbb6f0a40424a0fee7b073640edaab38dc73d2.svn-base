...
/**
 * A TaxicabNumber object stores an integer s that can be written in more than
 * one way as a sum of cubes along with two pairs of integers (a, b) and (c, d)
...
public class TaxicabNumber implements Comparable<TaxicabNumber> {
	...
	/**
	 * A member of this class is basically a struct that holds a taxicab number
	 * and the evidence that it is indeed such a number. a^3 + b^3 = s = c^3 +
	 * d^3
	 * 
	 * @param s
	 *            sum of cubes
	...
	public TaxicabNumber(int s, int a, int b, int c, int d) {
		...
		if (this.a == this.c)
			throw new HardyException("The two given ways of writing  " + s + " as the sum of two cubes are identical");
	}
	...
	public String toString() {
		return this.s + " = " + this.a + "^3 + " + this.b + "^3 = " + this.c + "^3 + " + this.d + "^3";
	}
	 ...
	 * 
	 * @param otherCab
	 * @return true iff the numbers in these representations are the same
	 *         (except for order)
	 */
